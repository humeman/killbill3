...
int main(int argc, char* argv[]) {
    ...
    PRINT_PADDED("Dungeon", DUNGEON_WIDTH);
    ...
    // Now the tunneling map
    PRINT_PADDED("Distance Map (Non-Tunneling)", DUNGEON_WIDTH);
    uint32_t distance;
    for (y = 0; y < dungeon.height; y++) {
        for (x = 0; x < dungeon.width; x++) {
            distance = dungeon.pathfinding_no_tunnel[x][y];
            if (distance == 0) printf("%c", CELL_TYPE_PC);
            // This one has the potential to have unconnectable cells (in our special dungeons)
            // To match the sample dungeons, I'm printing an X wherever that happens
            else if (distance == UINT32_MAX) {
                if (dungeon.cells[x][y].type != CELL_TYPE_STONE) printf("X");
                else printf(" ");
            }
            else printf("%d", distance % 10);
        }
        printf("\n");
    }

    // And the non-tunneling map
    PRINT_PADDED("Distance Map (Tunneling)", DUNGEON_WIDTH);
    for (y = 0; y < dungeon.height; y++) {
        for (x = 0; x < dungeon.width; x++) {
            distance = dungeon.pathfinding_tunnel[x][y];
            if (distance == 0) printf("%c", CELL_TYPE_PC);
            // This shouldn't be necessary here, unless we intentionally made a dungeon
            // with 255-hardness immutable stone surrounding a room (doesn't hurt to include)
            else if (distance == UINT32_MAX) {
                if (dungeon.cells[x][y].type != CELL_TYPE_STONE) printf("X");
                else printf(" ");
            }
