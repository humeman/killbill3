...
int main(int argc, char* argv[]) {
    ...
    if (prepare_args(argc, argv, &read, &write, &debug, &nummon, dungeon_path, monster_path, item_path)) {
        return 1;
    }
    ...
    if (read) {
        game.init_from_file(dungeon_path.c_str());
    }
    else {
        game.init_random();
    }
    ...
    if (write) {
        game.write_to_file(dungeon_path.c_str());
    }
    ...
    game.init_monster_defs(monster_path.c_str());
    game.init_item_defs(item_path.c_str());
...
int prepare_args(int argc, char* argv[], bool *read, bool *write, bool *debug, int *nummon, std::string &dungeon_path, std::string &monster_path, std::string &item_path) {
    int i;
    ...
    dungeon_path += home;
    dungeon_path += DEFAULT_DUNGEON_PATH;
    monster_path += home;
    monster_path += DEFAULT_MONSTER_PATH;
    item_path += home;
    item_path += DEFAULT_ITEM_PATH;
    *nummon = -1;
    for (i = 1; i < argc; i++) {
        if (d_path_next) {
            dungeon_path = argv[i];
        ...
        else if (strcmp(argv[i], "-l") == 0 || strcmp(argv[i], "--load") == 0) *read = true;
        else if (strcmp(argv[i], "-s") == 0 || strcmp(argv[i], "--save") == 0) *write = true;
        else if (strcmp(argv[i], "-d") == 0 || strcmp(argv[i], "--debug") == 0) *debug = true;
