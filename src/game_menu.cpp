...
std::string Game::run_menu() {
    ncpp::Plane *overlay = planes->get("overlay");
    ...
    unsigned long m_width = 3 * term_x / 4;
    // Scale this up to a multiple of 14.
    m_width += 14 - (m_width % 14);
    unsigned long m_height = m_width / 14;
    // Find out where that goes to be centered.
    unsigned long x_center = (term_x - m_width) / 2;
    unsigned long y_center = (term_y - m_height) / 2;
    ncpp::Plane *logo = planes->get("menu_michaelsoft", x_center, y_center, m_width, m_height);
    ...
    unsigned long size = MIN(term_x / 2, term_y);
    ncpp::Plane *bg = planes->get("menu_bg", 0, 0, size * 2, size);
    bg->move_top();
    NC_DRAW(bg, "ui_bg");
    nc->render();
    std::this_thread::sleep_for(std::chrono::seconds(1));
    ...
    ncpp::Plane *text = planes->get("menu_bg_text", term_x - 60, term_y / 2 - 1, 60, 2);
    text->move_top();
